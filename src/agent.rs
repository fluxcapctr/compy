...
/// Where the running app listens: in the user's private runtime directory, or, without one, in a
/// directory of their own under the home (never a shared /tmp, where any local account could reach it).
pub fn socket_path() -> PathBuf {
    let base = std::env::var_os("XDG_RUNTIME_DIR").filter(|v| !v.is_empty()).map(PathBuf::from).unwrap_or_else(|| {
        let dir = std::env::var_os("HOME").map(PathBuf::from).unwrap_or_else(std::env::temp_dir).join(".cache/compositor");
        let _ = std::fs::create_dir_all(&dir);
        #[cfg(unix)] { use std::os::unix::fs::PermissionsExt; let _ = std::fs::set_permissions(&dir, std::fs::Permissions::from_mode(0o700)); }
        dir
    });
    let user = std::env::var("USER").ok().filter(|u| !u.is_empty()).unwrap_or_else(|| format!("uid{}", unsafe { libc::getuid() }));
    base.join(format!("compositor-agent-{user}.sock"))
}
...
pub fn tools() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec { name: "select_layer", description: "Make a layer active, by id or name (an id is exact; with two layers of one name, the topmost is taken).", schema: obj(json!({"layer": {"type": "string", "description": "Layer id or exact name"}}), &["layer"]) },
        ToolSpec { name: "select_layers", description: "Select several layers at once, by id or name (the last one becomes active): what align_layers, distribute_layers and artboard_from_layers act on.", schema: obj(json!({"layers": {"type": "array", "items": {"type": "string"}}}), &["layers"]) },
        ...
        ToolSpec { name: "generative_expand", description: "Grow the canvas by pixels on each side and fill the new area from a text prompt (fal.ai, costs money). Returns the estimated cost without changing anything when estimate_only is true.", schema: obj(json!({"left": {"type": "integer"}, "right": {"type": "integer"}, "top": {"type": "integer"}, "bottom": {"type": "integer"}, "prompt": {"type": "string"}, "estimate_only": {"type": "boolean"}}), &["prompt"]) },
        ToolSpec { name: "generative_edit", description: "Change the active layer's pixels (or just the selected part of them) by instruction, such as 'make the shirt red' or 'give her a moustache', through an image edit model. The result lands as a new layer over the original, so both stay. model: 'local' runs Qwen-Image-2.1 on this machine, free and offline, and keeps the layer's own size; 'nano banana' (Google), 'gpt image' (OpenAI), 'flux' (Kontext), or a full fal id all go to fal and are billed. transparent: true asks for a cutout on a transparent background (GPT Image does this; it is used automatically).", schema: obj(json!({"prompt": {"type": "string"}, "count": {"type": "integer"}, "model": {"type": "string"}, "transparent": {"type": "boolean"}}), &["prompt"]) },
        ToolSpec { name: "generate_image", description: "Make a new picture from a text prompt as a new layer; size defaults to the canvas. model: 'local' runs Qwen-Image-2.1 on this machine, free and offline, but wants about a megapixel, so pass width and height near 1024x1024 rather than letting a large canvas set them; 'nano banana' (Google), 'gpt image' (OpenAI), 'flux', or a full fal id all go to fal and are billed. transparent: true makes a cutout (an object, a logo, a person) on a transparent background, the right choice for anything that will sit over other layers; GPT Image is used automatically for it.", schema: obj(json!({"prompt": {"type": "string"}, "width": {"type": "integer"}, "height": {"type": "integer"}, "model": {"type": "string"}, "transparent": {"type": "boolean"}}), &["prompt"]) },
        ...
        ToolSpec { name: "export", description: "Write the visible composite to a PNG, JPEG, WebP, GIF or AVIF file (by the path's extension; quality applies to JPEG and AVIF). Paths must be inside the user's home folder and outside its hidden folders; an existing file is not replaced unless overwrite is true.", schema: obj(json!({"path": {"type": "string"}, "quality": {"type": "number"}, "overwrite": {"type": "boolean"}}), &["path"]) },
        ToolSpec { name: "save", description: "Save the document (as a .comp project); a path is needed the first time. A new path must be inside the user's home folder and must not replace another project unless overwrite is true.", schema: obj(json!({"path": {"type": "string"}, "overwrite": {"type": "boolean"}}), &[]) },
        ToolSpec { name: "open", description: "Open an image, Photoshop file or .comp project (inside the user's home folder) in a new tab.", schema: obj(json!({"path": {"type": "string"}}), &["path"]) },
