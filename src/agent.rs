...
pub fn tools() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec { name: "filter", description: "Run a filter on the active layer: gaussian_blur (radius), motion_blur (angle, distance), add_noise (amount), unsharp_mask (amount %, radius, threshold), smart_sharpen (amount %, radius, noise %; sharpens brightness only), levels (black, white, gamma), exposure (exposure), hue_saturation (hue, saturation, lightness), color_balance (shadows, midtones, highlights: each [cyan_red, magenta_green, yellow_blue] from -100 to 100), brightness_contrast (brightness -150 to 150, contrast -50 to 100), vibrance (vibrance, saturation -100 to 100), black_white (reds, yellows, greens, cyans, blues, magentas in percent), photo_filter (color #rrggbb, density 1 to 100, preserve_luminosity), threshold (level 1 to 255), posterize (levels 2 to 255), shadows_highlights (shadows, highlights 0 to 100, radius), selective_color (range Reds/Yellows/Greens/Cyans/Blues/Magentas/Whites/Neutrals/Blacks, cyan, magenta, yellow, black -100 to 100, relative), channel_mixer (red, green, blue: each [from_red, from_green, from_blue, constant] in percent; monochrome), high_pass (radius), radial_blur (amount, method spin or zoom).", schema: obj(json!({"kind": {"type": "string"}, "range": {"type": "string"}, "cyan": {"type": "number"}, "magenta": {"type": "number"}, "yellow": {"type": "number"}, "black_ink": {"type": "number"}, "relative": {"type": "boolean"}, "red": {"type": "array"}, "green": {"type": "array"}, "blue": {"type": "array"}, "monochrome": {"type": "boolean"}, "method": {"type": "string"}, "radius": {"type": "number"}, "angle": {"type": "number"}, "distance": {"type": "number"}, "amount": {"type": "number"}, "threshold": {"type": "number"}, "noise": {"type": "number"}, "black": {"type": "number"}, "white": {"type": "number"}, "gamma": {"type": "number"}, "exposure": {"type": "number"}, "hue": {"type": "number"}, "saturation": {"type": "number"}, "lightness": {"type": "number"}, "shadows": {"type": "array"}, "midtones": {"type": "array"}, "highlights": {"type": "array"}, "brightness": {"type": "number"}, "contrast": {"type": "number"}, "vibrance": {"type": "number"}, "reds": {"type": "number"}, "yellows": {"type": "number"}, "greens": {"type": "number"}, "cyans": {"type": "number"}, "blues": {"type": "number"}, "magentas": {"type": "number"}, "color": {"type": "string"}, "density": {"type": "number"}, "preserve_luminosity": {"type": "boolean"}, "level": {"type": "number"}, "levels": {"type": "number"}}), &["kind"]) },
        ...
        ToolSpec { name: "layer_style", description: "Set the active layer's effects. Pass any of: drop_shadow {color, opacity, angle, distance, size}, inner_shadow {same}, outer_glow {color, opacity, size}, inner_glow {same}, bevel {style 0 inner 1 outer 2 emboss, depth, size, angle, altitude}, stroke {size, position 0 outside 1 inside 2 center, color, opacity}, color_overlay {color, opacity}, blend_if {this_black, this_white, under_black, under_white (0 to 255: the layer shows only where its own tones and the tones beneath fall in range), feather}. Colors as #rrggbb, opacities 0 to 1. An empty object clears the style.", schema: obj(json!({"drop_shadow": {"type": "object"}, "inner_shadow": {"type": "object"}, "outer_glow": {"type": "object"}, "inner_glow": {"type": "object"}, "bevel": {"type": "object"}, "stroke": {"type": "object"}, "color_overlay": {"type": "object"}, "blend_if": {"type": "object"}}), &[]) },
