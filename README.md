...
On a 50-megapixel, five-layer document the first frame takes about 180 ms (building the sharp halvings)
and every frame after that draws in 2 to 15 ms at any zoom, because each layer is sampled only over the
visible region and every offscreen buffer is sized to the window, not the document.
